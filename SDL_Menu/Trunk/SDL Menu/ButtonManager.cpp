...
void ButtonManager::DrawButton(SDL_Surface* screen)
{
	...
	for (m_vButtonsIT = m_vButtons.begin(); m_vButtonsIT != m_vButtons.end(); m_vButtonsIT++)
	{
		...
		m_pMyText->Print(screen, m_vButtonsIT->GetX() + 5, m_vButtonsIT->GetY() + (m_vButtonsIT->GetHeight() / 4), m_vButtonsIT->GetText().c_str(), bc.iRed, bc.iGreen, bc.iBlue, 0, 0, "MyFont");
