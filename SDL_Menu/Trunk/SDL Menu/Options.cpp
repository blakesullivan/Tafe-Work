...
Options::Options(MyText* mt)//NetworkClient* nc, MyText* mt)
{
	//m_pNetworkClient = nc;
	m_pMyText = mt;
	Init();
...
void Options::Init()
{
	...
	m_pButtonManager = new ButtonManager(m_pMyText);
...
void Options::Update(SDL_Surface* screen, float fDeltaTime)
{
	...
	m_pButtonManager->UpdateButton(mouseX, mouseY);
	if (m_bButtonHeld[SDL_BUTTON_LEFT] == true)
	{
		m_pButtonManager->ButtonPressed(mouseX, mouseY);
		if (m_pButtonManager->FindButton(buttonName)->GetPressed() == true)
		{
			if (m_pButtonManager->FindButton(buttonName)->GetText() == "Player")
			{
				m_pButtonManager->FindButton(buttonName)->SetText("");
	...
	//{
	//	m_pButtonManager->ButtonUnPressed(buttonHost);
	//	m_pButtonManager->ButtonUnPressed(buttonJoin);
	//}
}
...
void Options::Draw(SDL_Surface* screen, float fDeltaTime)
{
	...
	m_pButtonManager->DrawButton(screen);
	DrawText(screen);
}
...
{
	int iButtonPressed = m_pButtonManager->CheckButtonPressed();
	...
	if (m_iIsRunning != sceneRunning)
		{m_pButtonManager->ButtonUnPressed(iButtonPressed);}

	if (iButtonPressed == buttonName)
	{
		AddLetterToName();
		//m_pButtonManager->FindButton(buttonName)->SetText(m_pNetworkClient->GetName());
	}

	if (m_pButtonManager->FindButton(buttonHost)->GetPressed() == true)
	{
		m_pButtonManager->ButtonUnPressed(buttonHost);
		m_pButtonManager->FindButton(buttonHost)->SetClickable(false);
		m_pButtonManager->FindButton(buttonJoin)->SetClickable(false);
		//m_pNetworkClient->Send('h', m_pNetworkClient->GetName());
	}
	else if (m_pButtonManager->FindButton(buttonJoin)->GetPressed() == true)
	{
		m_pButtonManager->ButtonUnPressed(buttonJoin);
		m_pButtonManager->FindButton(buttonHost)->SetClickable(false);
		m_pButtonManager->FindButton(buttonJoin)->SetClickable(false);
...
void Options::AddMenuButtons()
{
	...
	m_pButtonManager->AddButton(300, 450, "Start Game", 100, 30, true, true, cN, cH, cD, false, scenePlay);
	m_pButtonManager->AddButton(300, 500, "Back", 100, 30, true, true, cN, cH, cD, false, sceneMenu);
}
...
void Options::AddOptions()
{
	...
	//m_pButtonManager->AddButton(200, 200, m_pNetworkClient->GetName(), 400, 30, true, true, cWhite, cGrey, cWhite, true, buttonName);
	bool bClick = true;
	//if (m_pNetworkClient->GetJoined() == true) {bClick = false;} else {bClick = true;}
	m_pButtonManager->AddButton(300, 250, "Host", 50, 30, bClick, true, cN, cH, cD, false, buttonHost);
	m_pButtonManager->AddButton(375, 250, "Join", 50, 30, bClick, true, cN, cH, cD, false, buttonJoin);
}
void Options::DrawText(SDL_Surface* screen)
{
	std::string sTemp = "";
	
	m_pMyText->Print(screen, 145, 210, "Name:");
	sTemp = "Network Status: " ;
	//if (m_pNetworkClient->GetJoined()) 
	//	{sTemp += "Joined";} 
	//else 
		{sTemp += "Not Connected";}
	m_pMyText->Print(screen, 250, 290, sTemp);
}

void Options::AddLetterToName()
{
