...
Square::Square()
{
	...
	SetForeColour(ccWhite);
	SetBackColour(ccBlack);
}

void Square::Init(int n, bool ie, int pnr, int r, int c, int sg, int fcol, int bcol)
{
	SetNum(n);
	SetIsUserNum(false);
	SetIsEmpty(ie);
...
void Square::SetForeColour(int fc)
{
	m_iForeColour = fc;
}
void Square::SetBackColour(int bc)
{
	m_iBackColour = bc;
}
...
int Square::GetForeColour()
{
	return m_iForeColour;
}
int Square::GetBackColour()
{
	return m_iBackColour;
}