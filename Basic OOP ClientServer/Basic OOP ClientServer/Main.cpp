		...
		while (iClientState != -1)
		{
			...
			if (iClientState == 0)//Refresh Server List page
			{
				...
				if (bReturn == true)
				{
					...
					if (cInput[0] == 'r' || cInput[0] == 'R')
					{
						...
						net.SendStruct(pData, srvadd, sizeof(*pData));
						...
						while (SDLNet_UDP_Recv(net.GetSocket(), net.GetPacket()))
						{
							...
							//if (cInput[0] == '!')
							srData = (serverResponsePacket*)net.GetPacket()->data;
							...
							if (srData->id = SERVER_RESPONSE)
							{
								...
								sprintf(sIP, "%d.%d.%d.%d", net.GetPacket()->address.host&0xff, (net.GetPacket()->address.host>>8)&0xff, (net.GetPacket()->address.host>>16)&0xff, (net.GetPacket()->address.host>>24)&0xff);
								//cout << sIP << endl;
								aipAddress[iNumServers] = net.GetPacket()->address;
					...
					else
					{
						...
						srvadd = net.ResolveHost(cInput, port);
			...
			else if (iClientState == 1) //Input name and connect
			{
				...
				if (bReturn == true)
				{
					...
					//SDLNet_UDP_Send(sd, -1, packet); /* This sets the p->channel */
					net.SendStruct((basePacket*)cpData, srvadd, sizeof(*cpData));
					...
					while (!connected)
					{
						while (SDLNet_UDP_Recv(net.GetSocket(), net.GetPacket()))
						{
							...
							nuData = (newUserPacket*)net.GetPacket()->data;
							iID = nuData->userID;
			...
			else if (iClientState == 2)
			{
				...
				if (bReturn == true)
				{
					...
					//packet->len = strlen((char *)packet->data) + 1;
					//SDLNet_UDP_Send(sd, -1, packet); /* This sets the p->channel */
					net.SendStruct((basePacket*)mData, srvadd, sizeof(*mData));
				...
				{
					//printf("%s\n", (char *)packet->data);
					messagePacket* smData = (messagePacket*)net.GetPacket()->data;
		...
		while (!quit)
		{
			...
			while (SDLNet_UDP_Recv(net.GetSocket(), net.GetPacket()))
			{
				...
				if (tempData->id == CLIENT_BROADCAST)//(cBuffer[0] == '@')
				{
					...
					//net.SendStruct(srData, srvadd, sizeof(srData));
					delete[] srData;
				}
				...
				//	sprintf(acUserNames[iNumConnected], token);
					aipAddress[iNumConnected].host = net.GetPacket()->address.host;
					aipAddress[iNumConnected].port = net.GetPacket()->address.port;
				...
				//	packet->len = strlen((char *)packet->data) + 1;
					...
					net.SendStruct((basePacket*)nuData, aipAddress[iNumConnected], sizeof(*nuData));
				...
				//		packet->len = strlen((char *)packet->data) + 1;
						//SDLNet_UDP_Send(sd, -1, packet); /* This sets the p->channel */
						net.SendStruct((basePacket*)smData, aipAddress[i], sizeof(*smData));
						delete[] smData;
					}
				//
				}
