...
namespace aStar
{
	 ...
	 */
	//AB any comments staring with //AB are changes to the original file

		//Declare constants
	//AB	const int mapWidth = 80, mapHeight = 60, tileSize = 10, numberPeople = 3;
	//		change these details to match your map tile size
		const int mapWidth = 25, mapHeight = 19, tileSize = 32, numberPeople = 3;
	...
	//AB	const walkable = 0, unwalkable = 1;// walkability array constants
	//		change this if you dont want to use these numbers
		const int walkable = 0, unwalkable = 9;// walkability array constants

		//Create needed arrays

	//AB	char walkability [mapWidth][mapHeight];
	//AB	this array is declared elswhere. You can delcare it here if you want to.
	...
	void SetWalkability(int x, int y, int walk);
	int GetXPath(int pathfinderID);
	int GetYPath(int pathfinderID);
	...
	//2.Quick Path Checks: Under some circumstances no path needs to
	//	be generated ...
	...
	//AB	change the G cost depending on the tile they walk on eg water vs road
		if (walkability[startX][startY]== 9)
			Gcost[startX][startY] = 10; //reset starting square's G value to 0
	// AB you may want to add in other values depending on what you use the tile value for
	...
	//5.Do the following until a path is found or deemed nonexistent.
		do
		{
			...
			//AB	while (!KeyDown(27));//reorder the binary heap
									...
									if (corner == walkable) 
									{
										...
										{	
											...
											if (abs(a-parentXval) == 1 && abs(b-parentYval) == 1)
												addedGCost = 14000;//cost of going to diagonal squares	
											else	
												addedGCost = 10;//cost of going to non-diagonal squares	

											//AB CHange the cost of walking on other tiles
											if (walkability[a][b]== 5)
												addedGCost += 10; 

											//	add more depending on what you use the the tile values for
										...
										else //If whichList(a,b) = onOpenList
										{
											...
											if (abs(a-parentXval) == 1 && abs(b-parentYval) == 1)
												addedGCost = 2400000;//cost of going to diagonal tiles	
											else	
	...
	*/
	...
	void SetWalkability(int x, int y, int walk)
	{
		walkability[x][y] = walk;
	}

	int GetXPath(int pathfinderID)
	{
		int temp = 0;
		temp = xPath[pathfinderID];
		return temp;
	}
	int GetYPath(int pathfinderID)
	{
		int temp = 0;
		temp = yPath[pathfinderID];
		return temp;
	}
