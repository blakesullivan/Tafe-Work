...
void Manager::CreateTiles()
{
	...
	for (unsigned int i = 0; i < uiLoop; i++)
	{
		...
		//SetWalkability(temp.GetGridX(), temp.GetGridY(), temp.GetPassable());
	}
}
		...
		for (m_vTilesIT = m_vTiles.begin(); m_vTilesIT != m_vTiles.end(); m_vTilesIT++)
		{
			if (m_vTilesIT->GetGridX() == iTempX &&  m_vTilesIT->GetGridY() == iTempY)
			{
				if(m_vTilesIT->GetPassable() == 0)
				{
					...
					{
						m_bg.Init(m_vTilesIT->GetX(), m_vTilesIT->GetY(), 0, 0, m_vTilesIT->GetGridX(), m_vTilesIT->GetGridY());
						bExit = true;
					}
				}
			}
...
void Manager::DrawPlayer(Player& play, SDL_Surface* playSurf)
{
	...
	sclip.y = play.GetPicRow() * Player::GetSize();
...
{
	//FindPath(1, m_bg.GetGridX(), m_bg.GetGridY(), m_player.GetGridX(), m_player.GetGridY());
	/*if (GetXPath(1) != m_bg.GetGridX())
	{
		m_bg.SetGridX(GetXPath(1));
	}
	if (GetYPath(1) != m_bg.GetGridY())
	{
		m_bg.SetGridY(GetYPath(1));
	}*/
	m_bg.SetX(m_bg.GetGridX() * Tile::GetSize());
	m_bg.SetY(m_bg.GetGridY() * Tile::GetSize());
}